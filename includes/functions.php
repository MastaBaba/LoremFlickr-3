...
function init() {
	...
	$filename = $site["path"] . $site["cache"];
	if (!file_exists($filename)) {
		mkdir($filename, 0755);
	}

	$filename = $site["path"] . $site["cacheSearch"];
	if (!file_exists($filename)) {
		mkdir($filename, 0755);
	}

	$filename = $site["path"] . $site["cacheOriginals"];
	if (!file_exists($filename)) {
		mkdir($filename, 0755);
	}

	$filename = $site["path"] . $site["cacheResized"];
	if (!file_exists($filename)) {
		mkdir($filename, 0755);
	}
...
function usageWithinLimit() {
	...
	$day = date("d", $now);
	$hour = date("H", $now);
	$usage = json_decode(file_get_contents($site["path"].'usage.json'), true);

	//Remove old data
	$past = $now - 60 * 24 * 60 * 60;
	$oldyear = date("Y", $past);
	...
	//Check and store
	if (!isset($usage["".$year]["".$month]["".$day]["".$hour])) {
		$usage["".$year]["".$month]["".$day]["".$hour] = 0;
	}
	if ($usage["".$year]["".$month]["".$day]["".$hour] < $site["keyLimit"]) {
		$usage["".$year]["".$month]["".$day]["".$hour]++;
		
		file_put_contents ($site["path"].'usage.json', json_encode($usage));
...
function moveToCache($url) {
	...
	$newFile = $site["cacheOriginals"].implode("_", $filenameComponents);
...
function thumbnail_box($filename, $img, $box_w, $box_h) {
	...
	$resizedFile = $site["cacheResized"].$pathParts['filename']."_".$box_w."_".$box_h.".jpg";
...
function addFilter($filename, $filter, $license, $owner) {
	...
	if ($filter == "g" || $filter == "p" || $filter == "red" || $filter == "green" || $filter == "blue") {
		$filteredFile = $site["cacheResized"].$pathParts['filename']."_".$filter.".jpg";
	}
	else {
		$filteredFile = $site["cacheResized"].$pathParts['filename']."_nofilter.jpg";
	}
